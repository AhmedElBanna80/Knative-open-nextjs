...
class Validator {
    ...
    async validateAppRouter() {
        const pagesDir = path_1.default.join(this.projectDir, 'pages');
        const appDir = path_1.default.join(this.projectDir, 'app');
        const srcPagesDir = path_1.default.join(this.projectDir, 'src', 'pages');
        const srcAppDir = path_1.default.join(this.projectDir, 'src', 'app');
        ...
        if (hasPagesDir) {
            ...
            const nonApiFiles = files.filter(f => f !== 'api' && f !== '_app.tsx' && f !== '_document.tsx' && f !== '_app.js' && f !== '_document.js' && f !== '_app.ts' && f !== '_document.ts');
        ...
        console.log('✅ App Router validation passed.');
    }
    async validatePPR() {
        ...
        for (const file of configFiles) {
            const filePath = path_1.default.join(this.projectDir, file);
        ...
        if (!hasPPR) {
            ...
            console.log('✅ PPR validation passed.');
        }
    }
}
